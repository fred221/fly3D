...
struct Point
{
	Point(float x, float y, float z, dtPolyRef ref):
		pos_x(x),
		pos_y(y),
		pos_z(z),
		d_ref(ref)
...
public:
	...
	dtStatus findNearestByHightPoly(const float* center, const float* halfExtents,
		const dtQueryFilter* filter,
	...
	dtStatus findRandomPointAroundCircle(dtPolyRef startRef, const float* centerPos, const float maxRadius,
										 ...
										 dtPolyRef* randomRef, float* randomPt) const;
	//加一个有最小半径的
	dtStatus findRandomPointRadius(dtPolyRef startRef, const float* centerPos, const float minRadius, const float maxRadius,
		const dtQueryFilter* filter, float(*frand)(float, float), dtPolyRef* randomRef, float* randomPt) const;
