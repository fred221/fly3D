													 ...
													 dtPolyRef* randomRef, float* randomPt) const
{
	...
	float distance = dtVdist2D(pt, centerPos);
	if (distance > maxRadius)
	{
		float min_vdis = FLT_MAX;
		const float* min_v = nullptr;
		for (int i = 0; i < randomPoly->vertCount; ++i)
		{
			v = &randomTile->verts[randomPoly->verts[i] * 3];
			float v_dis = dtVdist2D(v, centerPos);
			if (v_dis < maxRadius && min_vdis > v_dis)
			{
				min_v     = v;
				min_vdis  = v_dis;
			}
		...
		if (min_v && min_vdis < INT_MAX)
		{
			bool isSect = false;
			float u1 = 0, u2 = 0;
			dtlineCircleSectPoint(pt,min_v,centerPos,maxRadius, isSect,u1,u2);

			pt[0] += u1 * (min_v[0] - pt[0]);
			pt[2] += u1 * (min_v[2] - pt[2]);

			float random_r = dtVdist2D(pt, min_v) * frand();

			float dir[3] = { 0 };
			dtVsub(dir, pt, min_v);
			dtVnormalize(dir);
			float move_dis[3] = {0};
			dtVscale(move_dis, dir, random_r);
			dtVadd(pt, min_v, move_dis);
		}
		else
		{
			float sect_minA[3] = { FLT_MAX };
			float sect_minB[3] = { FLT_MAX };
			float min_dis = FLT_MAX;
			for (int i = 0; i < randomPoly->vertCount; ++i)
			{
				float* posA = &randomTile->verts[randomPoly->verts[i] * 3];
				float* posB = (i == randomPoly->vertCount - 1) ?
					&randomTile->verts[randomPoly->verts[0] * 3] :
					&randomTile->verts[randomPoly->verts[i+1] * 3];

				bool isSect = false;
				float u1 = 0, u2 = 0;
				dtlineCircleSectPoint(posA, posB, centerPos, maxRadius, isSect,u1,u2);

				if (!isSect) continue;
				
				float sectA[3] = { posA[0] + u1 * (posB[0] - posA[0]),posA[1], posA[2] + u1 * (posB[2] - posA[2]) };
				float sectB[3] = { posA[0] + u2 * (posB[0] - posA[0]),posA[1], posA[2] + u2 * (posB[2] - posA[2]) };
 				
				float dis_A = dtVdist2D(pt, sectA);
				float dis_B = dtVdist2D(pt, sectB);

				float min_dis_AB = dis_A < dis_B ? dis_A : dis_B;
				if (min_dis_AB < min_dis)
				{
					dtVcopy(sect_minA, sectA);
					dtVcopy(sect_minB, sectB);
					min_dis = min_dis_AB;
				}
			}
			...
			float centerArea = dtTriArea2D(sect_minA, sect_minB, centerPos);
			float pArea = dtTriArea2D(sect_minA, sect_minB, pt);

			if (min_dis != FLT_MAX && centerArea * pArea < 0)
			{
				float randomH = frand();
				float h_point[3] = 
				{
					sect_minA[0] + randomH * (sect_minB[0] - sect_minA[0]),
					sect_minA[1],
					sect_minA[2] + randomH * (sect_minB[2] - sect_minA[2])
				};
				
				dtVsub(dir, h_point, centerPos);
				dtVnormalize(dir);
				dtVscale(move_dis, dir, maxRadius);
				dtVadd(pt, centerPos, move_dis); 
			}
			else
			{
				float random_r = maxRadius * frand();
				dtVsub(dir, pt, centerPos);
				dtVnormalize(dir);
				dtVscale(move_dis, dir, random_r);
				dtVadd(pt, centerPos, move_dis);
			}
	...
	float h = 0.0f;
	dtStatus hstate = getPolyHeight(randomPolyRef, pt, &h);
	if (dtStatusFailed(hstate))
	{
		float closePos[3] = { 0 };
		bool posOverPoly = false;
		closestPointOnPoly(randomPolyRef, pt, closePos, &posOverPoly);
		h = closePos[1];
	}
	if (dtStatusFailed(status))
		return hstate;
...
dtStatus dtNavMeshQuery::findRandomPointRadius(dtPolyRef startRef, const float* centerPos, const float minRadius, const float maxRadius, const dtQueryFilter* filter, float(*frand)(float, float), dtPolyRef* randomRef, float* randomPt) const
{
	dtAssert(m_nav);
	...
	for (int i = 0; i < m_nav->getMaxTiles(); i++)
	{
		...
		tsum += area;
		const float u = frand(0, 1);
		if (u*tsum <= area)
			tile = t;
	}
	if (!tile || tile->header == NULL)
		return DT_FAILURE;
	
	static const int resultMax = 9999;
	int resultMaxCount = 0;
	dtPolyRef maxPolyRef[resultMax];
	findPolysAroundCircle(startRef, centerPos, maxRadius, filter, maxPolyRef, NULL, NULL, &resultMaxCount, resultMax);

	if (resultMaxCount <= 0)
	{
		return DT_FAILURE;;
	}
	std::vector<Point> randomVec;
	randomVec.reserve(100);
	randomVec.clear();

	for (int index = 0; index < resultMaxCount; ++index)
	{
		dtPolyRef curPolyRef = maxPolyRef[index];

		const dtPoly* bestPoly = 0;
		m_nav->getTileAndPolyByRefUnsafe(curPolyRef, &tile, &bestPoly);
		if (bestPoly == NULL)
		{
			continue;
		}

		int maxVertCount = tile->header->vertCount * 3;
		int curIndex = bestPoly->verts[0] * 3;
		if (curIndex < 0 || curIndex >= maxVertCount)
		{
			continue;
		}
		const float* v = &tile->verts[curIndex];

		float verts[3 * DT_VERTS_PER_POLYGON];
		float areas[DT_VERTS_PER_POLYGON];
		dtVcopy(&verts[0 * 3], v);
		for (int j = 1; j < bestPoly->vertCount; ++j)
		{
			v = &tile->verts[bestPoly->verts[j] * 3];
			dtVcopy(&verts[j * 3], v);
		}

		const float s = frand(0, 1);
		const float t = frand(0, 1);

		float pt[3];
		memset(pt, 0, sizeof(float) * 3);

		dtRandomPointInConvexPoly(verts, bestPoly->vertCount, areas, s, t, pt);
		if (dtVdist2D(centerPos, pt) < minRadius)
		{
			continue;
		}
		if (dtVdist2D(centerPos, pt) > maxRadius)
		{
			float dir[3] = { 0 };
			dtVsub(dir, pt, centerPos);
			dtVnormalize(dir);
			...
			{
				radius = frand(minRadius, maxRadius);
			}
			float distanceMove[3] = { 0 };
			dtVscale(distanceMove, dir, radius);
			dtVadd(pt, centerPos, distanceMove);
		}

		float h = 0.0f;
		dtStatus status = getPolyHeight(curPolyRef, pt, &h);
		if (dtStatusSucceed(status))
		{
			pt[1] = h;
		}
		Point point(pt[0], pt[1], pt[2], curPolyRef);
		randomVec.push_back(point);
	}
	...
	*randomRef = randomVec[0].d_ref;
	randomPt[0] = randomVec[0].pos_x;
	randomPt[1] = randomVec[0].pos_y;
	randomPt[2] = randomVec[0].pos_z;
...
// 增加一个包含高的
class dtFindNearestPolyByHightQuery : public dtPolyQuery
{
	const dtNavMeshQuery* m_query;
	const float* m_center;
	float m_nearestDistanceSqr;
...
public:
	dtFindNearestPolyByHightQuery(const dtNavMeshQuery* query, const float* center)
		: m_query(query), m_center(center), m_nearestDistanceSqr(FLT_MAX), m_nearestRef(0), m_nearestPoint()
	{
	}
...
dtStatus dtNavMeshQuery::findNearestByHightPoly(const float* center, const float* halfExtents,
	const dtQueryFilter* filter,
	dtPolyRef* nearestRef, float* nearestPt, bool& isFind) const
{
	dtAssert(m_nav);
	...
	dtFindNearestPolyByHightQuery query(this, center);
	...
	*nearestRef = query.nearestRef();
	...
	if (nearestPt && *nearestRef)
	{
		...
		/*const dtMeshTile* bestTile = 0;
		const dtPoly* bestPoly = 0;
		m_nav->getTileAndPolyByRefUnsafe(*nearestRef, &bestTile, &bestPoly);

		if (bestTile == NULL || bestTile->header == NULL || bestPoly == NULL)
		{
			return DT_FAILURE;
		}

		float areas[DT_VERTS_PER_POLYGON];
		const float s = (float)rand() / (float)RAND_MAX;
		const float t = (float)rand() / (float)RAND_MAX;

		float verts[3 * DT_VERTS_PER_POLYGON];

		int heardVertCount = bestTile->header->vertCount * 3;
		if (bestPoly->verts[0] * 3 >= heardVertCount)
		{
			return DT_FAILURE;
		}

		const float* v = &bestTile->verts[bestPoly->verts[0] * 3];

		dtVcopy(&verts[0 * 3], v);
		for (int j = 1; j < bestPoly->vertCount; ++j)
		{
			//若数组越界 直接return
			if (bestPoly->verts[j] * 3 >= heardVertCount)
			{
				return DT_FAILURE;
			}

			v = &bestTile->verts[bestPoly->verts[j] * 3];

			if (j >= DT_VERTS_PER_POLYGON)
			{
				return DT_FAILURE;
			}
			dtVcopy(&verts[j * 3], v);
		}
		dtRandomPointInConvexPoly(verts, bestPoly->vertCount, areas, s, t, nearestPt); */
		isFind = true;
	}
	return DT_SUCCESS;
}
