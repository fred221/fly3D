...
/*intersectSegmentTriangle(src, dst,
&verts[tris[j] * 3],
&verts[tris[j + 1] * 3],
&verts[tris[j + 2] * 3], t))
*/
...
bool OurGeom::loadMesh(rcContext* ctx, const std::string& filepath)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
...
bool OurGeom::loadGeomSet(rcContext* ctx, const std::string& filepath)
{
	char* buf = 0;
	...
	while (src < srcEnd)
	{
		...
		if (row[0] == 'f')
		{
			...
			if (*name)
			{
				//应该只有一行Mesh数据 ，直接改为 Obj 拼接， 不使用 name变量了
				size_t externsionPos = filepath.find_last_of('.');
				std::string baseName = filepath.substr(0, externsionPos);
				std::string realName = baseName.append(".obj");
				//if (!loadMesh(ctx, name))
				if(!loadMesh(ctx,realName.c_str()))
				{
					delete [] buf;
					return false;
				}
...
bool OurGeom::saveGeomSet(const BuildSettings* settings)
{
...
bool OurGeom::raycastMesh(float* src, float* dst, float& tmin)
{
	float dir[3];
	rcVsub(dir, dst, src);
...
void OurGeom::addOffMeshConnection(const float* spos, const float* epos, const float rad,
...
void OurGeom::deleteOffMeshConnection(int i)
{
	m_offMeshConCount--;
...
void OurGeom::drawOffMeshConnections(duDebugDraw* dd, bool hilight)
{
...
void OurGeom::addConvexVolume(const float* verts, const int nverts,
...
void OurGeom::deleteConvexVolume(int i)
{
	m_volumeCount--;
	m_volumes[i] = m_volumes[m_volumeCount];
}

void OurGeom::drawConvexVolumes(struct duDebugDraw* dd, bool /*hilight*/)
{
	dd->depthMask(false);
