...
struct rcSpan
{
	rcSpan& operator=(const rcSpan& span)
	{
		if (this != &span)
		{
			this->smin = span.smin;
...
struct rcVoexlSpan
{
	int smin;
	int smax;
	bool operator == (const rcVoexlSpan& other)
	{
		if (this == &other)
		{
			return false;
		}
		if (smin != other.smin || smax != other.smax)
		{
			return false;
		}
		return true;
...
const int Y_MAX = 4000;

typedef int				astar_int;
typedef size_t			astar_long;
typedef short int		astar_short;
typedef unsigned char	astar_char;
//-------------------------------------------------------------------
#endif // RECAST_H
