	...
	{ 
		for (int x = 0; x < w; ++x)
		{
			...
			while (s)
			{
				...
				{
					duCalcBoxColors(fcol, duRGBA(100, 100, 100, 255), duRGBA(100, 100, 100, 255));
				}
...
void duDebugDrawAllVoexl(struct duDebugDraw* dd,rcSpan*** span,const rcConfig& config ,const rcParam& param)
{
	if (!dd) return;
	...
	for (size_t tx = 0; tx < param.tweight; ++tx)
	{
		...
		{
			const float* origPos = &param.orig[(tx + ty * param.tweight) * 3];
			rcSpan** tileSpan = span[tx + ty * param.tweight];
			if (tileSpan == nullptr) 
			{
				continue;
			}
			for (int x = 0; x < w; ++x)
			{
				for (int y = 0; y <h; ++y)
				{
					float fx = origPos[0] + x * cs;
					float fz = origPos[2] + y * cs;
					const rcSpan* s = tileSpan[x + y * w];
					while (s)
					{
						duAppendBox(dd, fx, origPos[1] + s->smin*ch, fz, fx + cs, origPos[1] + s->smax*ch, fz + cs, fcol);
						s = s->next;
					}
				}
			}
...
void duDebugDrawInvVoexl(struct duDebugDraw* dd, rcInvSpan*** span, const rcConfig& config, const rcParam& param)
{
	if (!dd) return;
	...
	unsigned int fcol[6];
	duCalcBoxColors(fcol, duRGBA(255, 255, 255, 15), duRGBA(221, 160, 221, 15));
	...
	for (size_t tx = 0; tx < param.tweight; ++tx)
	{
		...
		{
			const float* origPos = &param.orig[(tx + ty * param.tweight) * 3];
			rcInvSpan** tileSpan = span[tx + ty * param.tweight];
			if (tileSpan == nullptr)
			{
				continue;
			}
			for (int x = 0; x < w; ++x)
			{
				for (int y = 0; y < h; ++y)
				{
					float fx = origPos[0] + x * cs;
					float fz = origPos[2] + y * cs;
					const rcInvSpan* s = tileSpan[x + y * w];
					while (s)
					{
						duAppendBox(dd, fx, origPos[1] + s->smin*ch, fz, fx + cs, origPos[1] + s->smax*ch, fz + cs, fcol);
						s = s->next;
