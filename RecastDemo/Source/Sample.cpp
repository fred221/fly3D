...
extern  const std::string meshesFolder;
unsigned int SampleDebugDraw::areaToCol(unsigned int area)
{
	...
	// Build :black
	case SAMPLE_POLYAREA_BUILD: return duRGBA(255, 255, 255, 255);
	// BLOCK1
	case SAMPLE_PARTITION_BLOCK1:return duRGBA(0, 0, 0, 255);
	// BLOCK2
	case SAMPLE_PARTITION_BLOCK2:return duRGBA(128, 128, 128, 255);
	// BLOCK3
	case SAMPLE_PARTITION_BLOCK3:return duRGBA(0, 128, 128, 255);
	// BLOCK4
	case SAMPLE_PARTITION_BLOCK4:return duRGBA(128, 0, 128, 255);
	// BLOCK5
	case SAMPLE_PARTITION_BLOCK5:return duRGBA(128, 128, 0, 255);
	// BLOCK6
	case SAMPLE_PARTITION_BLOCK6:return duRGBA(200, 200, 200, 255);
	// Unexpected : red
...
{
	m_geom = geom;
	const std::string& fileName = geom->getMesh()->getFileName();
	size_t extensionPos = fileName.find_last_of('.');
	if (extensionPos != std::string::npos)
	{
		std::string fileNameFix = fileName.substr(0,extensionPos);
		m_souceMeshName = fileNameFix.substr(meshesFolder.size() + 1,fileNameFix.size());
	}
...
std::string Sample::getSaveName(int saveType)
{
	std::string pathNav;
	if (saveType == 0) //二维数据
		pathNav = "navmesh_" + m_souceMeshName + ".bytes";
	else if (saveType == 1)  // 3d数据
		pathNav = "navmesh_" + m_souceMeshName + "3D.bytes";
