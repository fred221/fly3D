...
NavMeshTesterTool::NavMeshTesterTool() :
	...
	m_steerPointCount(0)
{
	m_filter.setIncludeFlags(SAMPLE_POLYFLAGS_ALL ^ SAMPLE_POLYFLAGS_DISABLED ^ SAMPLE_POLYFLAGS_PWNGS_BLOCK1 ^
		SAMPLE_POLYFLAGS_PWNGS_BLOCK2 ^
		SAMPLE_POLYFLAGS_PWNGS_BLOCK3 ^
		SAMPLE_POLYFLAGS_PWNGS_BLOCK4 ^
		SAMPLE_POLYFLAGS_PWNGS_BLOCK5 ^
		SAMPLE_POLYFLAGS_PWNGS_BLOCK6
	);
...
void NavMeshTesterTool::handleMenu()
{
	...
	if (imguiCheck("Pathfind Fly", m_toolMode == TOOLMODE_PATHFIND_FLY))
	{
		m_toolMode = TOOLMODE_PATHFIND_FLY;
		recalc();
	}
	...
	if (imguiCheck("Raycast3D", m_toolMode == TOOLMODE_RAYCAST3D))
	{
		m_toolMode = TOOLMODE_RAYCAST3D;
		recalc();
	}
	imguiSeparator();
...
void NavMeshTesterTool::recalc()
{
	...
	if (m_toolMode == TOOLMODE_PATHFIND_FLY)
	{
		if (m_sample)
		{
			Sample_TileMesh* sample = dynamic_cast<Sample_TileMesh*>(m_sample);
			if (sample)
			{
				float beginPos[3] = { 0 };
				float endPos[3] = { 0 };

				dtVcopy(beginPos, m_spos);
				dtVcopy(endPos, m_epos);

				beginPos[1] += 1.5;
				endPos[1] += 1.5;
				DetourAstar& detour = sample->getDetourAstar();
				BuildContext* ctx = m_sample->getContext();
				ctx->resetTimers();
				ctx->startTimer(RC_TIMER_TEMP);
				memset(m_flyPath, 0, sizeof(float) * 3000);
				m_flyPathLen = 0;
				detour.findPath(beginPos, endPos, m_flyPath, m_flyPathLen);
				ctx->stopTimer(RC_TIMER_TEMP);
				ctx->log(RC_LOG_ERROR, "fly find time is %.2fms ", ctx->getAccumulatedTime(RC_TIMER_TEMP) / 1000.0f);
			}
		}
	}
	else if (m_toolMode == TOOLMODE_PATHFIND_FOLLOW)
	{
		m_pathIterNum = 0;
		if (m_sposSet && m_eposSet && m_startRef && m_endRef)
		{
	...
	else if (m_toolMode == TOOLMODE_RAYCAST3D)
	{
		memset(m_hitPos, 0, sizeof(float) * 3);

		float startPos[3] = {0};
		float endPos[3] = {0};

		dtVcopy(startPos,m_spos);
		dtVcopy(endPos, m_epos);
		...
		if (sample)
		{
			m_hitResult = sample->getDetourAstar().rayFly(startPos, endPos, m_hitPos);
		}
	}
...
void NavMeshTesterTool::handleRender()
{
	...
	if (m_toolMode == TOOLMODE_PATHFIND_FLY)
	{
		if (m_flyPathLen > 0)
		{
			...
			for (int i = 0; i < m_flyPathLen; ++i)
				dd.vertex(m_flyPath[i * 3], m_flyPath[i * 3 + 1] + 0.1f, m_flyPath[i * 3 + 2], spathCol);
			dd.end();
			dd.depthMask(true);
		}
	}
	...
	else if (m_toolMode == TOOLMODE_RAYCAST3D)
	{
		if (m_hitResult)
		{
			...
			dd.begin(DU_DRAW_LINES, 2.0f);
			dd.vertex(m_spos[0], m_spos[1] + 3 , m_spos[2], hitCol);
			dd.vertex(m_hitPos[0], m_hitPos[1] , m_hitPos[2], hitCol);
			dd.end();
			dd.depthMask(true);
		}
	}
