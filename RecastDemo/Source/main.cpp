...
static SampleItem g_samples[] =
{
	...
	{ createTile, "Only Mesh" },
	{ createTempObstacle, "Temp Obstacles" },
};
static const int g_nsamples = sizeof(g_samples) / sizeof(SampleItem);
extern const string meshesFolder = "Meshes";
int main(int argc, char** argv)
{

	if (argc > 1)
	{
		string build_type = string(argv[1]);
		if (build_type == "autoBuild")
		{
			autoBuild(argv[2], static_cast<float>(atof(argv[3])), static_cast<float>(atof(argv[4])), static_cast<float>(atof(argv[5])));
			return 0;
		}
	}
	...
	while(!done)
	{
		...
		if (showSample)
		{
			...
			Sample* newSample = 0;
			if (imguiItem(g_samples[1].name.c_str()))
			{
				newSample = g_samples[1].create();
				if (newSample)
